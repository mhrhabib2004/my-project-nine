...
const PropertyShoing = () => {
  ...
  return (
    <div style={{ backgroundImage: 'url(https://static.vecteezy.com/system/resources/thumbnails/002/282/910/small/elegant-white-background-free-vector.jpg)' }} data-aos="zoom-out-left" className="hero bg-no-repeat bg-cover bg-center  min-h-screen bg-base-200">
      <Helmet><title>Home Press || Property</title></Helmet>
      <div className="hero-content flex-col lg:flex-row">
        <img data-aos="zoom-out-up" src={image} className="lg:max-w-xl  rounded-lg shadow-2xl" />
        <div>
          <h1 className="text-4xl font-bold ">{estate_title}</h1>
          <p className="py-3 lg:text-xl font-bold"><span className="text-xl text-green-800 text-center">Description :</span> {description}</p>
          ...
          <div className="mt-4 flex lg:gap-4  items-center mb-4">
          <span className="text-[#131313] lg:text-xl lg:font-bold"><span className="lg:text-xl text-green-800">facilities:</span></span>

            {
              Data.facilities.map((facilities) =>
                <>
                  <div className="">
                    <h2 className="lg:font-bold  text-[#474747]  text-center ">{facilities}</h2>
                  </div>
                </>
              )
            }
